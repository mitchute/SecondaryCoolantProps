...
class PropyleneGlycol(BaseFluid):
    ...
    @staticmethod
    def _viscosity_pg(temperature: Union[int, float]):
        return (
            exp(
                -293.07
                + (17494 / (temperature + 273.15))
                + 40.576 * log10((temperature + 273.15))
            )
        ) * 1000.0
    ...
    def viscosity(self, temperature: Union[int, float] = 0):
        ...
        c_w = abs(1 - c_g)
        mu = exp(
            c_w * log10(BaseFluid._viscosity_water(temperature))
            + c_g * log10(PropyleneGlycol._viscosity_pg(temperature))
            + a3 * (125 - temperature)

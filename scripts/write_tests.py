...
def write(fluid, prop, test_cases, err_tol):
    for t in test_cases:
        temp = t[0]
        fl_str = ""
        try:
        ...
        print(f"# PropsSI('{prop}', 'P', 101325, 'T', 273.15 + {temp}, '{fl_str.upper()}')")
        p = PropsSI(prop, 'P', 101325, 'T', 273.15 + temp, fl_str)
        delta = p * err_tol
        print(f'self.assertAlmostEqual(self.p.{props[prop]}({temp:0.1f}), {p:0.3e}, delta={delta:0.3e})')
        print()
